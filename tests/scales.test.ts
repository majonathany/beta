...
test('intervals', () => {
  expect(
    Note.getAbsoluteNote({ note: NormalizedNote.Db, octave: 3 }, { degree: 2, dOctave: 1 })
  ).toEqual({ note: NormalizedNote.Eb, octave: 4 })

  expect(
    Note.getAbsoluteNote({ note: NormalizedNote.Bb, octave: 3 }, { degree: 2, dOctave: 2 })
  ).toEqual({ note: NormalizedNote.C, octave: 6 })
})
